...
public class NeuralNetwork {
    ...
    public class BackPropagationNeuronWeightAdaptionRunnable extends Thread implements Runnable {
        ...
        final double[] input;
        final double networkError;

        public BackPropagationNeuronWeightAdaptionRunnable(int layer, int from, int to, double[][] neuronError, double[][] outputs, double[] input, double networkError) {
            this.layer = layer;
            this.from = from;
            this.to = to;
        ...
        public void run() {
            for (int neuron = from; neuron < to; neuron++) {
                ...
                for (int weight = 0; weight < weights[layer][neuron].length; weight++) {
                    if (layer - 1 >= 0)
                        weights[layer][neuron][weight] += learningRate * neuronError[layer][neuron] * outputs[layer - 1][weight] * activationFunction.derivative(outputs[layer][neuron]) * networkError;
                    else
                        weights[layer][neuron][weight] += learningRate * neuronError[layer][neuron] * input[weight] * activationFunction.derivative(outputs[layer][neuron]) * networkError;
                }
                biases[layer][neuron] += neuronError[layer][neuron] * learningRate * activationFunction.derivative(outputs[layer][neuron]) * networkError;
            }
        }
    }
    ...
    private NeuralNetwork(int[] layers, double[][][] weights, double[][] biases, double learningRate, double dropout, ActivationFunction activationFunction, LossFunction lossFunction, int totalTrainedEpochs) {
        this.layers = layers;
    ...
    /**
     ...
     @return this neural network object for method chaining
     */
    ...
    /**
     ...
     @return the current NeuralNetwork object, for method chaining
     */
    public NeuralNetwork setLossFunction(LossFunction lossFunction) {
        this.lossFunction = lossFunction;
        return this;
    }
    ...
    /**
     ...
     @return the modified neural network object with updated learning rate
     */
    ...
    /**
     ...
     @return the modified neural network with the updated weight initialization
     */
    ...
    public void backPropagation(double[] input, double[] targetOutput) {
        ...
        if(lossFunction != null)
            networkError = lossFunction.function(outputs[outputs.length - 1], targetOutput);
        ...
        for (int layer = neuronError.length - 2; layer >= 0; layer--) {
            ...
            for (int thread = 0; thread < numberOfThreads; thread++) {
                backPropagationThreads[thread] = new Thread(new BackPropagationNeuronWeightAdaptionRunnable(layer, (int)Math.round(neuronRange * thread), (int)Math.round(neuronRange * (thread + 1)), neuronError, outputs, input, networkError));
    ...
    /**
     Trains the neural network for the specified time in milliseconds.
     @param time the training time in milliseconds
     @throws RuntimeException if no training data is available
     */
    public void train(long time) {
        ...
        neuralNetworkStatusPrinter = new Thread(new Runnable() {
            ...
            private void updateProgressBar() {
                ...
                output += "] " + totalTrainedEpochs + " (" + (System.currentTimeMillis() - startTime) / 1000 + "s / " + time / 1000 + "s)";
            ...
            public void run() {
                ...
                System.out.println("\nCompleted with an accuracy of " + accuracy + "% after " + (int) (time / 1000) + "s and " + totalTrainedEpochs + " epochs");
            }
        });
    ...
    private void save() {
        save(new File("autosave" + totalTrainedEpochs + ".nn"));
    }
    ...
    public static NeuralNetwork load(File file)  {
        try (DataInputStream in = new DataInputStream(new FileInputStream(file))) {
            ...
            return new NeuralNetwork(layers, weights, biases, learningRate, dropout, activationFunction, lossFunction, totalTrainedEpochs);
        } catch (IOException e) {
            e.printStackTrace();
            System.exit(1);
            return null;
